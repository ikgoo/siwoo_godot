...
const CONSTELLATION_MIN_X = 150
const CONSTELLATION_MAX_X = 400
const CONSTELLATION_MIN_Y = 120
const CONSTELLATION_MAX_Y = 270
...
func _ready():
	...
	for child in get_children():
		if child is Sprite2D and child.name.is_valid_int():
			child.modulate = Color(1, 1, 1, 1)  # 회색으로 되돌림
