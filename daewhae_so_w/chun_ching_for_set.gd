...
const CONSTELLATION_MIN_X = 127
const CONSTELLATION_MAX_X = 255
const CONSTELLATION_MIN_Y = 169
const CONSTELLATION_MAX_Y = 279
...
func _ready():
	...
	if normal_stars:
		for star in normal_stars.get_children():
			if star is Sprite2D:
				star.modulate = Color(1, 1, 1, 1)  # 회색
	
	var line_segments = []  # 선들의 시작점과 끝점을 저장
	
	# 숫자로 된 이름을 가진 별들의 위치는 저장
	for child in get_children():
		if child is Sprite2D and child.name.is_valid_int():
		...
		if closest_star and closest_distance <= CLICK_DISTANCE:
			if closest_star.name.is_valid_int():  # 네임드 별인 경우
				...
				if Gamemaneger.stardust >= cost:  # 스타더스트가 충분한 경우에만
					Gamemaneger.stardust -= cost  # 매번 스타더스트 차감 (조건문 제거)
...
func _on_star_clicked(star_node):
	if star_node in clicked_named_stars:
		return
		
	# 첫 번째 별을 클릭할 때만 스타더스트 확인
	if clicked_named_stars.size() == 0:
		var parent = get_parent()
		var cost = parent.constellation_info[parent.current_constellation]["cost"]
		
		if Gamemaneger.stardust < cost:
			return  # 스타더스트가 부족하면 클릭 불가
		
		# 스타더스트가 충분하면 차감
		Gamemaneger.stardust -= cost
	
	clicked_named_stars.append(star_node)
	emit_signal("named_star_clicked", total_stars, clicked_named_stars.size())

func cancel_last_click():
	if clicked_named_stars.size() > 0:
		clicked_named_stars.pop_back()
