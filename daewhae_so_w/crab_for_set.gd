...
const CONSTELLATION_MIN_X = 200
const CONSTELLATION_MAX_X = 450
const CONSTELLATION_MIN_Y = 150
...
func _ready():
	...
	var placed_positions = []  # 이미 배치된 별들의 위치를 저장
	var line_segments = []  # 선들의 시작점과 끝점을 저장
	
	# 네임드 별들의 위치와 선들의 정보 저장
	for child in get_children():
		if child is Sprite2D and child.name.is_valid_int():
			...
			for i in range(child.points.size() - 1):
				line_segments.append([child.points[i] + child.position, child.points[i + 1] + child.position])
	
	# Node2D 아래의 모든 Sprite2D 노드들의 위치를 랜덤하게 설정
	if normal_stars:
		for star in normal_stars.get_children():
			if star is Sprite2D:
